<template>
  <v-container>
    <v-row justify="center" align="center" class="mt-3">
      <v-col cols="10" xs="5" sm="7 " md="6" lg="5">
        <h2 class="text-h3 mb-9" justify="center" align="center">
          Iniciar sesión
        </h2>
        <div class="mb-9">
          <p class="mb-3">Iniciar sesión como paciente</p>
          <v-btn
            x-large
            elevation="0"
            raised
        ...
        <div>
          <p class="mb-3">También puedes ingresar como especialista</p>
          <v-btn
  ...
  methods: {
    ...
    async saveDataSourcesByUsers(dataSource, user, accessToken) {
      ...
      try {
        ...
        const startTime =
          dataSaving != null ? dataSaving.date : startTimeOrigin;
        ...
        if (res) {
          if (res.data) {
            ...
            res.data.bucket.forEach(async (obj, index) => {
              try {
                let value;
                if (name === "com.google.step_count.delta") {
    ...
    getState(obj) {
      ...
      if (value) {
        ...
        if (
          value >= dd.minAcept &&
          (value < dd.minSalud || value > dd.maxSalud) &&
          value <= dd.maxAcept &&
          value !== 0
        ) {
          return "yellow";
      ...
      try {
        ...
        res.session.forEach(async (obj, index) => {
          try {
            ...
            if (sleepObj.value !== null && sleepObj.value > 0) {
              await this.$fire.firestore
                .collection("dataSet")
                .doc()
                .set({
            ...
            if (sleepDeepObj.value !== null && sleepDeepObj.value > 0) {
              await this.$fire.firestore
                .collection("dataSet")
                .doc()
                .set({
